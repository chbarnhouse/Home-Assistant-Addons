...
const ASSET_TYPES_API_ENDPOINT = "asset_types"; // Changed endpoint
...
// Renamed component
function ManageAssetTypesSection({ assetTypes = [], onUpdate }) {
  ...
  useEffect(() => {
    ...
    const validTypes = assetTypes.filter(
  ...
  const handleAddType = async (event) => {
    ...
    if (!trimmedName) {
      setError("Asset type name cannot be empty."); // Changed text
      return;
    }
    if (typesList.some((t) => t.toLowerCase() === trimmedName.toLowerCase())) {
      setError(`Asset type "${trimmedName}" already exists.`); // Changed text
      return;
    }
    ...
    setError(null);
    try {
      const result = await callApi(ASSET_TYPES_API_ENDPOINT, {
        // Changed endpoint
        method: "POST",
        body: JSON.stringify({ name: trimmedName }),
      });
      if (result && (result.success || result.asset_types)) {
        // Changed check
        notify(`Asset type "${trimmedName}" added successfully.`, "success"); // Changed text
        setNewTypeName("");
        if (onUpdate && result.asset_types) {
          // Changed check
          onUpdate(result.asset_types); // Changed check
        } else {
          onUpdate([...typesList, trimmedName]); // Optimistic
        }
      } else {
        throw new Error(result?.error || "Failed to add asset type."); // Changed text
      }
    } catch (err) {
      console.error("Error adding asset type:", err); // Changed text
      setError(err.message || "An unknown error occurred.");
      notify("Failed to add asset type.", "error"); // Changed text
    } finally {
      setIsAdding(false);
    }
  };
  ...
  const handleSaveEdit = async (event) => {
    ...
    if (
      ...
      setError(`Asset type "${trimmedValue}" already exists.`); // Changed text
      return;
    }
    ...
    try {
      ...
      if (result && (result.success || result.asset_types)) {
        // Changed check
        notify(
          `Asset type renamed to "${trimmedValue}" successfully.`, // Changed text
          "success"
        );
        setEditingTypeName(null);
        if (onUpdate && result.asset_types) {
        ...
        throw new Error(result?.error || "Failed to rename asset type."); // Changed text
      }
    } catch (err) {
      console.error("Error saving asset type edit:", err); // Changed text
      setError(err.message || "An unknown error occurred.");
  ...
  const handleConfirmDelete = async (typeToDelete) => {
    ...
    try {
      ...
      if (result && (result.success || result.asset_types)) {
        // Changed check
        notify(
          `Asset type "${typeToDelete}" deleted successfully.`, // Changed text
          "success"
        );
        if (onUpdate && result.asset_types) {
          // Changed check
        ...
        throw new Error(result?.error || "Failed to delete asset type."); // Changed text
      }
    } catch (err) {
      console.error("Error deleting asset type:", err); // Changed text
      setError(err.message || "An unknown error occurred.");
      notify("Failed to delete asset type.", "error"); // Changed text
    } finally {
      setDeletingType(null);
    }
  };
    ...
    <Paper elevation={2} sx={{ p: 3, mb: 3 }}>
      ...
      <List dense disablePadding>
        {typesList.length > 0 ? (
          ...
          <ListItem dense>
            <ListItemText primary="No asset types defined yet." />
          </ListItem>
        )}
      </List>
    </Paper>
...
ManageAssetTypesSection.propTypes = {
...
export default ManageAssetTypesSection;
